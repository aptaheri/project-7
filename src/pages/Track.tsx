...
type Status = 'loading' | 'ok' | 'unauthorized' | 'missing-key' | 'error'
...
export default function Track() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(`/api/track?key=${encodeURIComponent(key)}`)
        if (cancelled) return
        if (res.status === 401) {
  ...
  return (
    <div className="track">
      ...
      <div className="track-panel">
        {status === 'missing-key' && (
          <>
            <p className="track-panel-title">Access token required</p>
            <p className="track-panel-note">
              This page needs a key. Open it as <code>/track?key=…</code>
            </p>
          </>
        )}
        ...
        {status === 'unauthorized' && (
          <>
            <p className="track-panel-title">Invalid token</p>
            <p className="track-panel-note">That key was not accepted.</p>
          </>
        )}
        ...
        {status === 'ok' && !latest && (
          <>
            ...
            <p className="track-panel-note">
              Waiting for the first location from OwnTracks.
            </p>
          </>
        )}
